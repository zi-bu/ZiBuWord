    ...
    internal class Program
    {
        ...
        private static async Task ImportJsonFileToNewTable(string jsonFilePath, string tableName, string connectionString)
        {
            try
            {
                ...
                Console.WriteLine($"  成功读取并乱序处理 {jsonWords.Count} 个单词条目");
        ...
        private static async Task AddRandomOrderToTable(string tableName, string connectionString)
        {
            try
            {
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    if (totalCount == 0)
                    {
                        Console.WriteLine($"  表 {tableName}_Words 中没有记录，跳过随机序号生成");
                        return;
                    }
                    
                    Console.WriteLine($"  开始为 {tableName}_Words 表中的 {totalCount} 个记录生成随机序号");

                    // SQL Server方法：使用ROW_NUMBER和NEWID()来生成随机序号
                    string updateSql = $@"
                        WITH RandomCTE AS (
                            SELECT 
                                Id, 
                    ...
                    using (var command = new SqlCommand(updateSql, connection))
                    {
                        ...
                        Console.WriteLine($"  已成功为 {rowsAffected} 个记录生成随机序号");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"  生成随机序号时出错: {ex.Message}");
                if (ex.InnerException != null)
                {
                    Console.WriteLine($"  内部异常: {ex.InnerException.Message}");
                }
