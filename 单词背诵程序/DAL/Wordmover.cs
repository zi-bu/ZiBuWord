...
namespace DAL
{
    ...
    /// mouse:新增了一个GetWords方法，用于随机获取一个单词;<br/>
    /// 本类施工人员：mouse;
    /// </summary>
                ...
                case formid.CET4:
                    {
                        ...
                        var word = db.CET4.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                case formid.CET6:
                    {
                        int count = rd.Next(0, db.CET6.Count() - 1);
                        var word = db.CET6.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                case formid.初中:
                    {
                        int count = rd.Next(0, db.初中.Count() - 1);
                        var word = db.初中.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                ...
                case formid.高中:
                    {
                        ...
                        var word = db.高中.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                case formid.考研:
                    {
                        int count = rd.Next(0, db.考研.Count() - 1);
                        var word = db.考研.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                case formid.托福:
                    {
                        int count = rd.Next(0, db.托福.Count() - 1);
                        var word = db.托福.Skip(count).FirstOrDefault();
                        return word.word;
                    }
                default:
                    return null;
